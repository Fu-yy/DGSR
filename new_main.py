...
if __name__ == '__main__':
    ...
    if opt.record:
        log_file = f'{current_dir}/results/{opt.data}_ba_{opt.batchSize}_G_{opt.gpu}_dim_{opt.hidden_size}_ulong_{opt.user_long}_ilong_{opt.item_long}_' \
    ...
    if opt.model_record:
        model_file = f'{current_dir}/{opt.data}_ba_{opt.batchSize}_G_{opt.gpu}_dim_{opt.hidden_size}_ulong_{opt.user_long}_ilong_{opt.item_long}_' \
                     f'US_{opt.user_short}_IS_{opt.item_short}_La_{args.last_item}_UM_{opt.user_max_length}_IM_{opt.item_max_length}_K_{opt.k_hop}' \
                     f'_layer_{opt.layer_num}_l2_{opt.l2}'

    # loading data
    data = pd.read_csv(os.path.join(current_dir, 'Data', opt.data + '.csv'))
    user = data['user_id'].unique()
    item = data['item_id'].unique()
    user_num = len(user)
    item_num = len(item)
    train_root = os.path.join(current_dir, 'Newdata',
                              opt.data + '_' + str(opt.item_max_length) + '_' + str(opt.user_max_length) + '_' + str(
                                  opt.k_hop), 'train')
    test_root = os.path.join(current_dir, 'Newdata',
                             opt.data + '_' + str(opt.item_max_length) + '_' + str(opt.user_max_length) + '_' + str(
                                 opt.k_hop), 'test')

    val_root = os.path.join(current_dir, 'Newdata',
                            opt.data + '_' + str(opt.item_max_length) + '_' + str(opt.user_max_length) + '_' + str(
                                opt.k_hop), 'val')
